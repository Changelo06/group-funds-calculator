...
async function withPending(key, control, pendingLabel, action) { if (pendingMutations.has(key)) return; pendingMutations.add(key); const previousContent = control?.innerHTML; if (control) { control.disabled = true; if (pendingLabel) control.textContent = pendingLabel; control.setAttribute("aria-busy", "true"); } setAppLoading(key, pendingLabel || "Saving changes…"); try { return await action(); } finally { pendingMutations.delete(key); setAppLoading(key); if (control) { control.disabled = false; if (pendingLabel !== "") control.innerHTML = previousContent; control.removeAttribute("aria-busy"); } } }
