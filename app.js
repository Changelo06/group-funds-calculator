...
byId("generate-receipt")?.remove(); document.body.insertAdjacentHTML("beforeend", `<div class="app-loader" id="app-loader" role="status" aria-live="polite" hidden><i></i><span id="app-loader-label">Loading shared funds…</span></div>`);
...
function setAppLoading(key, label) { const loader = byId("app-loader"); if (label) activeLoading.set(key, label); else activeLoading.delete(key); const latestLabel = [...activeLoading.values()].at(-1); loader.hidden = !latestLabel; if (latestLabel) byId("app-loader-label").textContent = latestLabel; }
