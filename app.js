...
function showInitialSkeleton(errorMessage = "") { const skeleton = byId("initial-skeleton"); if (!skeleton) return; document.body.classList.add("initial-loading"); skeleton.hidden = false; skeleton.classList.toggle("load-failed", Boolean(errorMessage)); byId("initial-skeleton-label").textContent = errorMessage || "Loading shared funds…"; byId("initial-load-retry").hidden = !errorMessage; }
function finishInitialSkeleton() { const skeleton = byId("initial-skeleton"); if (!skeleton) return; document.body.classList.remove("initial-loading"); skeleton.classList.remove("load-failed"); skeleton.classList.add("is-leaving"); window.setTimeout(() => { skeleton.hidden = true; skeleton.classList.remove("is-leaving"); }, 180); }
...
async function loadState(silent = false, checkHealth = !healthChecked) { if (stateLoadPromise) return stateLoadPromise; const isInitialLoad = !silent && document.body.classList.contains("initial-loading"); if (!silent) { if (isInitialLoad) showInitialSkeleton(); setAppLoading("state", "Loading shared funds…"); } stateLoadPromise = (async () => { try { const nextState = await api("/api/state"); state = nextState; lastStateSyncAt = Date.now(); if (sharedFundId && state.funds.some(fund => fund.id === sharedFundId)) activeFundId = sharedFundId; render(); if (checkHealth) { try { await refreshHealth(silent); } catch { document.querySelector(".connection").classList.remove("online"); byId("connection-status").textContent = "Shared data unavailable"; healthChecked = false; } } if (isInitialLoad) finishInitialSkeleton(); } catch (error) { document.querySelector(".connection").classList.remove("online"); byId("connection-status").textContent = "Shared data unavailable"; healthChecked = false; if (isInitialLoad) showInitialSkeleton(error.message || "Could not load shared expense data."); else if (!silent) showToast(error.message || "Could not reach the shared expense data."); } finally { if (!silent) setAppLoading("state"); stateLoadPromise = null; } })(); return stateLoadPromise; }
byId("initial-load-retry").addEventListener("click", () => { showInitialSkeleton(); loadState(); });
