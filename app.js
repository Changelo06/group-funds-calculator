...
byId("fund-form").addEventListener("submit", async event => {
  ...
  await withPending(pendingKey, saveButton, editingFundId ? "Saving…" : "Creating…", async () => { try { state = editingFundId ? await api(`/api/funds/${editingFundId}`, {method:"PATCH",body:JSON.stringify(payload)}) : await api("/api/funds", {method:"POST",body:JSON.stringify(payload)}); if (!editingFundId) { addedFundId = state.funds.find(fund => !priorFundIds.has(fund.id))?.id || ""; window.setTimeout(() => { addedFundId = ""; }, 520); } const message = editingFundId ? "Split fund updated." : "Split fund added for the group."; closeModals(); render(); logTiming("T8 render completed", startedAt); showToast(message); } catch(error) { showToast(error.message); } });
});
