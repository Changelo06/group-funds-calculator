...
let state = { members: [], funds: [] }, activeRoute = "overview", activeFilter = "all", activeSort = "newest", activeOwnership = "all", editingFundId = null, activeFundId = null, currentReceipt = null, calculatorExpression = "";
...
const receiptInput = byId("receipt-input"), detailReceipt = byId("detail-receipt"), editFundButton = byId("edit-fund"); receiptInput.accept = "image/png,image/jpeg,.jpg,.jpeg,.png"; receiptInput.closest(".receipt-field").querySelector("div > span").innerHTML = "Attach receipt <em>optional</em>"; receiptInput.closest(".receipt-field").querySelector("div > small").textContent = "JPG, JPEG, or PNG · up to 5 MB."; detailReceipt.querySelector("img")?.remove(); editFundButton.innerHTML = "✎"; editFundButton.setAttribute("aria-label", "Edit fund"); editFundButton.title = "Edit fund";
byId("generate-receipt")?.remove(); document.body.insertAdjacentHTML("beforeend", `<div class="app-loader" id="app-loader" role="status" aria-live="polite" hidden><i></i><span id="app-loader-label">Loading shared funds…</span></div><section class="initial-skeleton" id="initial-skeleton" role="status" aria-live="polite" aria-label="Loading Group Funds Calculator"><aside class="skeleton-sidebar"><i class="skeleton-line skeleton-brand"></i><i class="skeleton-line skeleton-profile"></i><i class="skeleton-line skeleton-nav"></i><i class="skeleton-line skeleton-nav"></i><i class="skeleton-line skeleton-nav short"></i></aside><main class="skeleton-main"><div class="skeleton-mobile-header"><i class="skeleton-line skeleton-brand"></i><i class="skeleton-circle"></i></div><div class="skeleton-content"><i class="skeleton-line skeleton-overline"></i><i class="skeleton-line skeleton-title"></i><i class="skeleton-line skeleton-subtitle"></i><div class="skeleton-stats"><i></i><i></i><i></i></div><div class="skeleton-section"><i class="skeleton-line skeleton-section-title"></i><i class="skeleton-fund"></i><i class="skeleton-fund"></i><i class="skeleton-fund"></i></div></div><div class="initial-load-message"><i></i><span id="initial-skeleton-label">Loading shared funds…</span><button type="button" id="initial-load-retry" hidden>Retry</button></div></main></section>`); document.body.classList.add("initial-loading");
byId("initial-skeleton")?.remove(); document.body.classList.remove("initial-loading");
document.body.insertAdjacentHTML("beforeend", `<div class="fund-operation" id="fund-operation" role="status" aria-live="polite" hidden><span class="fund-operation-mark"><i></i><i></i><i></i></span><span><b id="fund-operation-title">Creating split fund</b><small id="fund-operation-copy">Adding it to the group</small></span></div>`);
...
const postedAge = fund => { const date = new Date(postedAt(fund)); if (Number.isNaN(date.getTime())) return "recently"; const today = new Date(), todayStart = new Date(today.getFullYear(), today.getMonth(), today.getDate()), postedStart = new Date(date.getFullYear(), date.getMonth(), date.getDate()), days = Math.max(0, Math.round((todayStart - postedStart) / 86400000)); return days === 0 ? "today" : `${days} day${days === 1 ? "" : "s"} ago`; };
const posterName = fund => (state.members.find(member => member.id === (fund.createdById || "member-1")) || { name:"Chan" }).name;
const sortFunds = (funds, sort = activeSort) => [...funds].sort((a,b) => { if (sort === "amount-high") return Number(b.total) - Number(a.total) || postedAt(b).localeCompare(postedAt(a)); if (sort === "amount-low") return Number(a.total) - Number(b.total) || postedAt(b).localeCompare(postedAt(a)); return sort === "oldest" ? postedAt(a).localeCompare(postedAt(b)) : postedAt(b).localeCompare(postedAt(a)); });
...
let stateLoadPromise = null, lastStateSyncAt = 0, healthChecked = false, initialStatePending = true;
let fundOperationTimer = 0, addedFundId = "";
...
function setAppLoading(key, label) { const loader = byId("app-loader"); if (label) activeLoading.set(key, label); else activeLoading.delete(key); const latestLabel = [...activeLoading.values()].at(-1); loader.hidden = !latestLabel || byId("profile-modal")?.classList.contains("profile-gate-loading"); if (latestLabel) byId("app-loader-label").textContent = latestLabel; }
function setFundOperation(kind, active) { const operation = byId("fund-operation"); if (!operation) return; clearTimeout(fundOperationTimer); if (!active) { operation.classList.remove("show"); fundOperationTimer = window.setTimeout(() => { operation.hidden = true; }, 180); return; } const creating = kind === "create"; byId("fund-operation-title").textContent = creating ? "Creating split fund" : "Deleting split fund"; byId("fund-operation-copy").textContent = creating ? "Adding it to the group" : "Removing it from the group"; operation.dataset.action = kind; operation.hidden = false; requestAnimationFrame(() => operation.classList.add("show")); }
function beginFundRemoval(fundId) { document.querySelectorAll(`[data-fund-id="${fundId}"]`).forEach(row => row.classList.add("fund-row-leaving")); }
function cancelFundRemoval(fundId) { document.querySelectorAll(`[data-fund-id="${fundId}"]`).forEach(row => row.classList.remove("fund-row-leaving")); }
function showProfileGateLoading(errorMessage = "") { const modal = byId("profile-modal"), failed = Boolean(errorMessage); modal.classList.add("profile-gate-loading"); modal.classList.toggle("profile-gate-error", failed); byId("profile-modal-title").textContent = failed ? "RM331 needs a moment" : "Preparing RM331…"; document.querySelector("#profile-modal .profile-picker-copy").textContent = failed ? errorMessage : "Loading your shared group."; byId("profile-picker-list").innerHTML = failed ? `<button class="profile-load-retry" type="button" data-retry-profile-load>Try again</button>` : `<span class="profile-gate-loader" aria-hidden="true"><i></i><i></i><i></i></span>`; document.body.classList.add("profile-required"); document.querySelector(".app-shell").setAttribute("aria-hidden", "true"); modal.hidden = false; }
function finishProfileGateLoading() { const modal = byId("profile-modal"); modal.classList.remove("profile-gate-loading", "profile-gate-error"); }
async function withPending(key, control, pendingLabel, action) { if (pendingMutations.has(key)) return; pendingMutations.add(key); const previousContent = control?.innerHTML, fundAction = key === "fund:create" ? "create" : key.startsWith("fund-delete:") ? "delete" : ""; if (control) { control.disabled = true; if (pendingLabel) control.textContent = pendingLabel; control.setAttribute("aria-busy", "true"); } if (fundAction) setFundOperation(fundAction, true); else setAppLoading(key, pendingLabel || "Saving changes…"); try { return await action(); } finally { pendingMutations.delete(key); if (!fundAction) setAppLoading(key); if (fundAction) setFundOperation(fundAction, false); if (control) { control.disabled = false; if (pendingLabel !== "") control.innerHTML = previousContent; control.removeAttribute("aria-busy"); } } }
...
async function loadState(silent = false, checkHealth = !healthChecked) { if (stateLoadPromise) return stateLoadPromise; const isInitialLoad = !silent && initialStatePending; if (!silent) { if (isInitialLoad) showProfileGateLoading(); setAppLoading("state", "Loading shared funds…"); } stateLoadPromise = (async () => { try { const nextState = await api("/api/state"); state = nextState; lastStateSyncAt = Date.now(); if (sharedFundId && state.funds.some(fund => fund.id === sharedFundId)) activeFundId = sharedFundId; if (isInitialLoad) { initialStatePending = false; finishProfileGateLoading(); } render(); if (checkHealth) { try { await refreshHealth(silent); } catch { document.querySelector(".connection").classList.remove("online"); byId("connection-status").textContent = "Shared data unavailable"; healthChecked = false; } } } catch (error) { document.querySelector(".connection").classList.remove("online"); byId("connection-status").textContent = "Shared data unavailable"; healthChecked = false; if (isInitialLoad) showProfileGateLoading(error.message || "Could not load shared expense data."); else if (!silent) showToast(error.message || "Could not reach the shared expense data."); } finally { if (!silent) setAppLoading("state"); stateLoadPromise = null; } })(); return stateLoadPromise; }
...
document.addEventListener("click", event => { const route = event.target.closest("[data-route]"); if (route) { event.preventDefault(); showRoute(route.dataset.route); return; } if (event.target.closest("[data-open-fund]") || event.target.closest("#mobile-add")) return openFundModal(); if (event.target.closest("[data-open-member]")) return openMemberModal(); const closeButton = event.target.closest("[data-close-modal]"); if (closeButton) { const modal = closeButton.closest(".modal-backdrop"); if (modal?.id === "receipt-preview-modal") { modal.hidden = true; return; } if (modal?.id === "fund-modal" && fundDraftHasChanges() && !window.confirm("Discard the unsaved changes to this split fund?")) return; if (modal?.id === "fund-modal") fundDraftSnapshot = ""; return closeModals(); } const filter = event.target.closest(".filter"); if(filter) { activeFilter = filter.dataset.filter; renderFunds(); return; } const row = event.target.closest("[data-fund-id]"); if(row) { activeFundId = row.dataset.fundId; renderDetail(); } });
...
byId("delete-fund").addEventListener("click", async () => { const fund = state.funds.find(item => item.id === activeFundId); if (!fund || !confirm(`Delete “${fund.title}”? This cannot be undone.`)) return; beginFundRemoval(fund.id); byId("detail-modal").hidden = true; await withPending(`fund-delete:${fund.id}`, byId("delete-fund"), "Deleting…", async () => { try { state = await api(`/api/funds/${fund.id}`, {method:"DELETE"}); activeFundId = null; render(); showToast("Split fund deleted."); } catch(error) { cancelFundRemoval(fund.id); renderDetail(); showToast(error.message); } }); });
...
const fundReceiverId = fund => fund.splitMode === "itemized" ? fund.payerId : fundCreatorId(fund);
const memberUnpaidFunds = memberId => state.funds.filter(fund => !settled(fund) && fundReceiverId(fund) !== memberId && payableMemberIds(fund).includes(memberId) && !fund.payments?.[memberId]);
const profileNotificationCount = memberId => memberUnpaidFunds(memberId).length;
const notificationCountLabel = count => count > 9 ? "9+" : String(count);
...
let activeSplitMode = "equal", itemizedDraft = [], itemizedGuests = [], fundDraftSnapshot = "";
...
function fundDraftSignature() { const items = [...document.querySelectorAll("[data-item-row]")].map(row => ({ id:row.dataset.itemRow, name:row.querySelector("[data-item-name]")?.value.trim() || "", amount:row.querySelector("[data-item-amount]")?.value || "", memberIds:[...row.querySelectorAll("[data-item-member]:checked")].map(input => input.value).sort() })); return JSON.stringify({ mode:activeSplitMode, title:byId("fund-title")?.value.trim() || "", description:sanitizeDescriptionHtml(byId("fund-description")?.innerHTML || ""), date:byId("fund-date")?.value || "", total:byId("fund-total")?.value || "", memberIds:[...document.querySelectorAll(".member-choice input:checked")].map(input => input.value).sort(), payerId:byId("itemized-payer")?.value || "", items, guests:itemizedGuests.map(guest => ({ id:guest.id, name:guest.name })), receipt:currentReceipt || "" }); }
function rememberFundDraft() { fundDraftSnapshot = fundDraftSignature(); }
function fundDraftHasChanges() { return Boolean(fundDraftSnapshot) && fundDraftSnapshot !== fundDraftSignature(); }
...
async function openFundModal(fund = null, requestedMode = null) {
  ...
  renderItemizedRows();
  byId("all-members-caption").textContent = `${people.length} people`; byId("fund-modal").hidden = false; setSplitMode(selectedMode); rememberFundDraft(); byId("fund-title").focus();
}
...
byId("fund-form").addEventListener("submit", async event => {
  ...
  const isEditing = Boolean(editingFundId), fundName = byId("fund-title").value.trim() || "this split fund";
  if (isEditing && !fundDraftHasChanges()) return showToast("No changes to save.");
  if (!window.confirm(isEditing ? `Save changes to “${fundName}”?` : `Create “${fundName}” for the group?`)) return;
  ...
  const saveButton = byId("save-fund-button"), pendingKey = editingFundId ? `fund:${editingFundId}` : "fund:create", priorFundIds = new Set(state.funds.map(fund => fund.id));
  await withPending(pendingKey, saveButton, editingFundId ? "Saving…" : "Creating…", async () => { try { state = editingFundId ? await api(`/api/funds/${editingFundId}`, {method:"PATCH",body:JSON.stringify(payload)}) : await api("/api/funds", {method:"POST",body:JSON.stringify(payload)}); if (!editingFundId) { addedFundId = state.funds.find(fund => !priorFundIds.has(fund.id))?.id || ""; window.setTimeout(() => { addedFundId = ""; }, 520); } const message = editingFundId ? "Split fund updated." : "Split fund added for the group."; fundDraftSnapshot = ""; closeModals(); render(); logTiming("T8 render completed", startedAt); showToast(message); } catch(error) { showToast(error.message); } });
});
...
function renderDetail() {
  ...
  const hasReceipt = Boolean(fund.receipt || fund.hasReceipt); detailReceipt.hidden = false; detailReceipt.disabled = !hasReceipt; detailReceipt.classList.toggle("has-receipt", hasReceipt); detailReceipt.classList.toggle("no-receipt", !hasReceipt); detailReceipt.querySelector("span").innerHTML = hasReceipt ? "See attached receipt <b>↗</b>" : "No receipt attached";
...
byId("profile-modal").insertAdjacentHTML("afterbegin", `<p class="profile-gate-brand" aria-hidden="true">RM331</p>`);
document.querySelector("#profile-modal .eyebrow").textContent = "RM331";
...
function fundsHtml(funds, emptyTitle, emptyCopy) {
  ...
  return funds.map(fund => {
    ...
    const summary = done ? "Settled" : `${remaining} to pay`, compactAge = postedAge(fund) === "today" ? "Today" : postedAge(fund);
    ...
    const mine = fundCreatorId(fund) === profile?.id, title = `${escapeHtml(fund.title)}<em class="fund-creator"> · ${escapeHtml(posterName(fund))}</em>`; return `<button class="fund-row ${mine ? "posted-by-you" : ""} ${done ? "is-settled" : ""} ${fund.id === addedFundId ? "fund-row-entering" : ""} profile-${profileState.tone}" data-fund-id="${fund.id}" type="button"><span class="fund-icon">${fund.icon || "◈"}</span><span class="fund-copy"><strong>${done ? `<s>${title}</s>` : title}</strong><small>${compactAge} · ${summary} · ${count}/${people} paid</small></span><span class="fund-meta"><b>${dateLabel(fund.date)}</b>${people} member${people === 1 ? "" : "s"}</span><span class="payment-progress"><span class="progress-caption"><span>${count}/${people} paid</span><b>${Math.round(people ? count / people * 100 : 0)}%</b></span><span class="progress"><i style="width:${people ? count / people * 100 : 0}%"></i></span></span><span class="fund-amount">${money(fund.total)}</span><span class="row-arrow">›</span></button>`;
  }).join("");
}
...
function renderMembers() {
  ...
  document.querySelector(".members-intro p").textContent = "Scroll to browse each member’s payment profile.";
  byId("members-list").innerHTML = state.members.map((member, index) => {
    const canManage = canManageMember(member), primaryPayment = paymentMethods(member)[0], paymentReady = Boolean(primaryPayment), unpaidCount = profileNotificationCount(member.id);
    ...
    const actions = `<button class="view-payment-button" type="button" data-view-payment-profile="${member.id}">${paymentReady ? "View payment QR" : "Set up payment"}</button><button class="select-account-button" type="button" data-select-profile="${member.id}">Use account</button>${canManage ? `<button class="edit-member-button" type="button" data-edit-member="${member.id}" aria-label="Edit ${escapeHtml(member.name)} profile">✎</button>` : ""}`;
    return `<article class="member-payment-card pipol-card" data-theme="${escapeHtml(member.theme || "maya-black")}">${unpaidCount ? `<span class="member-id-unpaid" aria-label="${unpaidCount} unpaid split funds">${notificationCountLabel(unpaidCount)} to pay</span>` : ""}<section class="payment-card-surface"><div class="payment-card-header"><span>RM331 PAY</span><span>${paymentReady ? "PAYMENT PROFILE" : "SETUP REQUIRED"}</span></div><div class="payment-card-chip" aria-hidden="true"><i></i><i></i></div><div class="payment-card-photo">${avatar}</div><div class="payment-card-copy"><small>${escapeHtml(member.label || "HOUSE MEMBER")}</small><b>${escapeHtml(member.nickname || member.name)}</b>${member.bio ? `<p>${escapeHtml(member.bio)}</p>` : ""}</div><div class="payment-card-method"><small>PAY TO</small><b>${escapeHtml(primaryPayment?.label || "Payment setup required")}</b><span>${memberFundCount(member.id)} shared fund${memberFundCount(member.id) === 1 ? "" : "s"}</span></div></section><div class="member-payment-actions">${actions}</div></article>`;
...
function openProfilePicker() {
  ...
  modal.classList.remove("profile-gate-loading", "profile-gate-error");
  byId("profile-modal-title").textContent = "Who’s using RM331?";
  document.querySelector("#profile-modal .profile-picker-copy").textContent = "Choose your profile to continue.";
...
document.addEventListener("click", event => {
  if (event.target.closest("[data-retry-profile-load]")) { initialStatePending = true; loadState(); return; }
