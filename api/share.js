...
module.exports = (request, response) => {
  const forwardedHost = String(request.headers["x-forwarded-host"] || request.headers.host || "localhost").split(",")[0].trim(), forwardedProtocol = String(request.headers["x-forwarded-proto"] || (forwardedHost === "localhost" || forwardedHost.startsWith("localhost:") ? "http" : "https")).split(",")[0].trim(), origin = `${forwardedProtocol}://${forwardedHost}`;
  const requestUrl = new URL(request.url, origin), fundId = requestUrl.searchParams.get("fund") || "", fundTitle = (requestUrl.searchParams.get("title") || "Shared split fund").slice(0, 100), creator = (requestUrl.searchParams.get("creator") || "RM331 member").slice(0, 60), date = (requestUrl.searchParams.get("date") || "").slice(0, 40), title = [fundTitle, creator, date].filter(Boolean).join(" · "), description = `Split fund created by ${creator}${date ? ` on ${date}` : ""}.`;
  const appUrl = new URL("/", origin); appUrl.searchParams.set("fund", fundId); appUrl.hash = "funds";
